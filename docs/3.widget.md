...
| `actions`             | Preact children | Adds host controls to the end of the bar under the composer.          |
...
every integration. React and Vue hosts must therefore create `actions` and `emptyActions`
with `h()` from `preact`.
...
and stored conversations. A settings or history page replaces those two with a back arrow
...
At the trailing edge: the context meter, then your `actions`. The gate button appears only
...
button: a pill with the button on the same line while the message is one line, and a
rounded box with the button under the field once it is more than one. Your `actions` stay
where they are on every page — use them for host chrome such as a collapse button.
...
inherited styles and host-provided children.
...
- context tokens and accumulated cost details, with an amber near-limit warning
...
Agentak registers `image` and `checkpoint` itself because the Pi transcript can emit
them. An unknown element name renders nothing instead of breaking the transcript.
