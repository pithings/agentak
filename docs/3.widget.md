...
| `actions`             | Preact children | Adds host controls to the end of the chat header.                     |
...
The header shows the conversation title first and every button after it, in this order:
new conversation, stored conversations, settings, then your `actions`. A settings or
history page replaces those buttons with a back arrow in front of the title, so your
`actions` stay where they are on every page.
...
- `/model` and `/new` commands with keyboard selection and completion
