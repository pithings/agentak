...
`encryptedStorage(inner, secret?)` takes a second argument that decides which names hold a
secret. The default is `isSecret`, which is every `api-key:*` name.
...
- `encryptedStorage(storage, secret?)` to seal the secrets in any store
