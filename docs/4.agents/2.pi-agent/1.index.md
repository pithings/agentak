...
Create the session once, outside a render function. A fresh session chooses no provider or
model. If the user sends a message first, Agentak holds it, opens settings, and sends it
after a usable model is selected.
...
| `storage`       | `PiStorage`                        | Keeps keys and picker choices. Shared page memory by default.                 |
