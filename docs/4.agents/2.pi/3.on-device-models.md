...
For tool calling, the LFM2.5 rows and Granite 4.1 3B are the strongest for their size. A
Qwen3.5 row calls a tool well only with thinking enabled; with thinking off its published
function-calling score falls by about half.

All ten accept text. A local model can stream text, reasoning, and tool calls when its chat template supports
